...
def simulate_Randomized_Greedy_Algorithm(start_zone, randomness=3, top=20):
    '''Greedy algorithm with probability for simulation
    and compute expectation'''
    ...
    matrix_prob_PU_DO_t0.index = matrix_prob_PU_DO_t0.index.astype(int)
    matrix_prob_PU_DO_t0.columns = matrix_prob_PU_DO_t0.columns.astype(int)
    matrix_prob_PU_DO_t0 = matrix_prob_PU_DO_t0.drop([0], axis=1).drop([int(i) for i in [0]])

    top_dropoff_zone_t0 = pd.DataFrame(matrix_prob_PU_DO_t0.idxmax(axis=1))
        
    zones = matrix_prob_PU_DO_t0.index
    ...
    while total_time >= 0:
        result_path.append(temp_zone)
        # compute highest pro drop off zone
        prob_dist_PU_DO = np.array(matrix_prob_PU_DO_t0.loc[temp_zone].to_list())        
        try:
            dropoff_zone = np.random.choice(zones,replace=True, size=1 , p=prob_dist_PU_DO)[0]
        except:
            dropoff_zone = top_dropoff_zone_t0.loc[temp_zone][0]
        result_path.append(dropoff_zone)
        if dropoff_zone > 262:
            dropoff_zone = 260
        ...
        try:
            ...
            print(temp_zone,dropoff_zone)
#             dropoff_trip_time =  matrix_zone_distance.iloc[temp_zone,dropoff_zone-3]
            
            print("Oh no!\n the data cleaning guys' fault\nthat include me...  XD")
        total_trip_time = total_trip_time + dropoff_trip_time # add trip time to total
        total_time = total_time - dropoff_trip_time # subtract the trip time from the day
